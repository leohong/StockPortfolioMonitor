...
from src.services.stock_service import load_stock, refresh
...
def stock_detail():
    ...
    with connect(settings.database):
        pass
    st.title("台股技術分析儀表板")
    st.caption("第一階段 · 證交所官方原始行情 · 三層互動圖")
    ...
    st.caption(f"共 {len(data)} 個交易日 · 成交量：股 · 成交金額：新臺幣 · 擷取時間：{latest.retrieved_at}")
    ...
    st.plotly_chart(stock_chart(data, selected), width="stretch", config={"displaylogo": False, "scrollZoom": True})
    st.caption("MA 與 RSI 先以完整歷史計算，再套用顯示區間；缺少暖機資料時保持空值。價格未還原權息。")
