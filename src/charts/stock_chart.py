...
def stock_chart(data, selected="120D"):
    ...
    fig = make_subplots(rows=3, cols=1, shared_xaxes=True, vertical_spacing=0.035, row_heights=[0.58, 0.22, 0.20])
    ...
    fig.add_trace(go.Scatter(x=dates, y=visible.rsi14, name="相對強弱指標 RSI14", hovertemplate="%{y:.4f}<extra>RSI14</extra>"), row=3, col=1)
    for value in (30, 50, 70):
        fig.add_hline(y=value, line_dash="dot", line_color="#888", row=3, col=1)
    ...
    fig.update_layout(xaxis=dict(anchor="y3", showticklabels=True, tickformat="%Y/%m/%d", hoverformat="%Y/%m/%d",
        range=[str(dates.iloc[0] - timedelta(days=1)), str(dates.iloc[-1] + timedelta(days=1))]))
    fig.update_layout(height=820, hovermode="x unified", hoversubplots="axis", margin=dict(l=40, r=20, t=40, b=30), legend=dict(orientation="h"))
    return fig
