...
def test_market_regime_is_structured_and_missing_breadth_stays_missing():
    result = calculate_market_regimes(benchmark(), "m2-regime-v1")[-1]
...
def test_risk_off_and_insufficient_history_are_explicit():
    assert calculate_market_regimes(benchmark(-1), "m2-regime-v1")[-1].regime == "RISK_OFF"
    short = calculate_market_regimes(benchmark(count=30), "m2-regime-v1")[-1]
...
def test_future_bars_do_not_change_as_of_regime():
    ...
    before = regime_as_of(history.iloc[:140], as_of, "m2-regime-v1")
    future = benchmark(-1, 20)
    future["market_date"] = [history.market_date.iloc[-1] + timedelta(days=index+1) for index in range(len(future))]
    after = regime_as_of(pd.concat([history, future], ignore_index=True), as_of, "m2-regime-v1")
