...
第九階段可從個股詳情下載目前分析快照 CSV，並按需產生 3200×2200 高解析度 PNG。PNG 直接使用頁面同一份 Plotly 五層圖與快照，包含七張證據卡、市場階段、支撐壓力、重要事件、官方來源及日期。PDF 依計畫保留為選配，尚未實作。
...
V3 M4 直接重用上述官方行情、法人與融資融券資料。Participation 結合量比、成交金額、收盤位置及突破／跌破背景；高量本身只會標示異常，不會被稱為出貨。Flow Persistence 分別保存外資、投信、自營商的 1／3／5／10／20 日淨額及 10 日方向持續度。Positioning 保存融資與融券變化、價格報酬、成交金額背景及 20 日槓桿背離；所有門檻均可在 `config/settings.yaml` 調整。
...
測試離線執行，使用官方封存資料與獨立暫存資料庫。數學測試用人工數列僅存在於測試中。驗收結果請見 `docs/PHASE1_ACCEPTANCE.md` 至 `docs/PHASE9_ACCEPTANCE.md`。
